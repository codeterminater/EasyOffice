...
namespace EasyOffice.Providers.NPOI
{
    public static class NPOIHelper
    {
            ...
            , Dictionary<string, IEnumerable<Picture>> pictureReplacements)
        {
            ...
            if ((stringReplacements?.Count ?? 0) > 0)
                placeholders.AddRange(stringReplacements.Select(x => x.Key));
            if ((pictureReplacements?.Count ?? 0) > 0)
                placeholders.AddRange(pictureReplacements.Select(x => x.Key));
